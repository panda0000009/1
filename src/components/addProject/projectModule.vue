...
<template>
	<el-dialog
		...
		<div
			...
			<div class="projectOpe h-10">
				<!-- <div
					class="projectOpeItemAdd cursor-pointer"
					@click="
						(isHistory = false),
							(tableChecked = false),
							(delBtnText = '批量删除')
				...
				<div
					class="projectOpeItemHistory cursor-pointer"
			...
			<div class="projectNew">
				...
				<div class="projectNewXmjbx">
					<div class="flex items-center">
						...
						<div class="flex items-center">
							<span
								class="rangeLabel ml-12"
								v-if="historyItem.rangeType.includes('区县')"
								>区县单元选择</span
							>
							<el-select
								v-if="historyItem.rangeType.includes('区县')"
							...
							<span
								class="rangeLabel ml-9"
								v-if="historyItem.rangeType.includes('街道')"
								>街道单元选择</span
							>
							<el-select-v2
								v-if="historyItem.rangeType.includes('街道')"
							...
							<span
								class="rangeLabel ml-9"
								v-if="historyItem.rangeType.includes('网格')"
								>网格单元选择</span
							>
							<el-select-v2
								v-if="historyItem.rangeType.includes('网格')"
		...
		<div class="projectNew" v-if="!showDetail && (!props.show || !isHistory)">
			<div class="projectNewContent">
				...
				<div class="projectNewStep">步骤{{ step > 3 ? 3 : step }}/3</div>
				<div
					class="projectStep"
					:style="{
						backgroundImage: `url(${
			...
			<div class="projectNewXmjbx">
				...
				<img
					class="h-[33px]"
					src="@/assets/img/deductionEngine/xzjszb.png"
					alt=""
					v-if="step === 2" />
				<div class="flex items-center">
					<img
						...
						alt=""
						v-if="step === 3 || step === 4 || step === 5" />
					<div class="flex items-center" v-if="step === 3">
				...
				<div
					class="projectNewXmjbxText mt-[10px]"
					v-if="[2, 3, 4, 5].includes(step)">
					{{
						step === 2
							? '请选择需要计算的城市要素指标，可多选'
							: step === 3
			...
			<div class="projectStepOne" v-if="step === 1">
				<el-form
					...
					<el-form-item label="推演间隔" prop="jianJie">
						<el-select
					...
					<el-form-item label="推演算法" prop="schemeId">
						<el-select
			...
			<!-- 第三步 -->
			<div class="projectStepOne projectStepTwo" v-if="[3].includes(step)">
			...
			<!-- 第四步 -->
			<div class="projectStepOne projectStepTwo" v-if="[4, 5].includes(step)">
			...
			<div class="projectStepOneBtn select-none" v-if="!resultShow">
				...
				<div class="flex items-center" v-if="step === 4">
					...
					<div
						class="projectStepOneBtnItem cursor-pointer ml-[76px] mr-[67px]"
						@click="handleClose">
						退出弹窗，后台继续计算
					</div>
					<div
...
<script setup>
	...
	const nextHandle = async () => {
		...
		if (step.value === 2 && checkList.value.length === 0) {
	...
	const handleStart = async () => {
		...
		if (!addForm.value.indicatorIds) {
			ElMessage({ message: '请先选择指标', type: 'error' });
			return;
		}
		step.value = 4;
		try {
			const res = await cityApi.projectBuild({
				...addForm.value
				// 	adjustData: JSON.stringify(ad),
				// 	baseYear: addForm.value.baseYear,
				// 	catalogId: selectHeader.value[0]['pid'] || '',
				// 	cjr: useStore.userInfo?.username || '',
				// 	indicatorIds: ad.map(item => item.indicatorId).join(','),
				// 	name: addForm.value.name,
				// 	predictYear: addForm.value.predictYear,
				// 	rangeType: addForm.value.rangeType,
				// 	remark: addForm.value.remark,
				// 	schemeId: addForm.value.schemeId
			});
			if (res.resultCode === 'S') {
				adjustData.value = {};
