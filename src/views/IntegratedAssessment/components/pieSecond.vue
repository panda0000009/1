...
<script setup>
	...
	const chartRefPieSecond = ref(null); // 第二张的第一个饼图
	let chartPieSecond = null;

	// 第二张的第一个饼图
	const chartPieSecondFn = () => {
		chartPieSecond = echarts.init(chartRefPieSecond.value);
		const optionsData = [
			{
				name: '安全指标',
				value: 30,
		...
		chartPieSecond.setOption(option);
		window.addEventListener('resize', () => chartPieSecond.resize());
	};
	...
	onMounted(() => {
		chartPieSecondFn(); // 第二张的第一个饼图
	});

	onBeforeUnmount(() => {
		chartPieSecond && chartPieSecond.dispose();
	});
</script>
<style scoped>
	.chartPieImage {
