				...
				<div class="w-full h-full left">
					...
					<div class="pl-[1.6875rem] pr-[0.4375rem]">
						<div>
							...
							<div v-show="titleStates.project" class="scenario-section">
								<div class="scenario-tabs">
									<div
										...
										class="scenario-tab"
										:class="{
											active: activeScenario === scenario.value
										}"
							...
							<div
								v-show="titleStates.vector"
								class="vector-statistics-section">
...
<script setup>
	...
	const getCatalogGetById = async id => {
		try {
			...
			if (res.resultCode === 'S' && res.data?.tree.length) {
				...
				handleTargetChange(targetList.value[0]);
			}
		} catch (error) {
	...
	const handleScenarioChange = async value => {
		...
		if (value == 'spatial') {
			...
			await initProjects('YJLD');
		}
		if (value == 'economy') {
			activeScenario.value = value;
			await initProjects('YJLD_CYZX');
		}
	};
	...
	const getZb = async () => {
		try {
			const res = await cityApi.resultTree({
				...
				array_indicatorId: '',
				eq_indicatorId: '', // 指标ID
				eq_projectId: selectedProjectInfo.value, // 推演项目ID
	...
	const handleTargetChange = value => {
		...
		let item = targetList.value.find(m => m.value == value.nodeId);
		// 根据目标更新相关数据
		lxList.value = item.children.map(m => {
			return {
				...m,
				value: m.nodeId,
				label: m.nodeName
		...
		if (warningValue2.value) {
			...
			if (sortDirection.value) {
				...
				vals = [...filterDatas].sort(
					(a, b) => b.predictValue - a.predictValue
				);
			} else {
				// 降序
				vals = [...filterDatas].sort(
					(a, b) => a.predictValue - b.predictValue
				);
			}
			...
			warningList.value = vals.map((m, index) => {
				...
				return {
					...
					value: m.predictValue, // parseFloat((ratios[index] * 100).toFixed(1)) + '%', // numScore,
					slide: m.predictValue, // parseFloat((ratios[index] * 100).toFixed(1)),
	...
	const initRadarStatisticInfo = async () => {
		try {
			...
			if (res?.data) {
				...
				reportInfo.value.indicatorName = indicatorInfo.nodeName;
				...
				if (cityItem) {
					...
					reportInfo.value.cityVal = cityItem.predictValue.toFixed(2) + indicatorInfo?.unit;
					alertLevel.value = {
						"0": "安全",
						"1": "警告",
						"2": "危险",
